...
var voronoi_opacity = .3;
var voronoi_coarseness = 3;
...
function moveCenters () {
	...
	if (all_finalized){
		...
		stop_update = true;
		document.getElementById('voronoi').disabled = false;
	}
}
...
function reset(){
	stream = false;
	document.getElementById('movecenters').disabled = false;
	document.getElementById('togglestream').disabled = false;
	document.getElementById('voronoi').disabled = true;
	document.getElementById('final').innerHTML = "Unfinalized";
	document.getElementById('voronoitext').innerHTML = "<b>Warning</b>: takes ~40 seconds.";
	stop_update = false;
	algo_started = false;
	new_points = false;
	points = new Array();
...
function generateVoronoi(){
	document.getElementById('voronoi').disabled = true;
	document.getElementById('voronoitext').innerHTML = "<img src='voronoi.gif' />Generating...";
	for (var i = 0; i < svg_size.width; i+=voronoi_coarseness){
		for (var j = 0; j < svg_size.height; j+=voronoi_coarseness){
			var center_candidate = null;
			var center_candidate_dist = 999999;
			var svg_centers = svg.selectAll('.center')[0]
			for (var k = 0; k < centers.length; k++){
				var svg_center = svg_centers[k];
				center_dist = eucDist({x:i,y:j},{x:svg_center.cx.baseVal.value,y:svg_center.cy.baseVal.value});
				if (center_dist < center_candidate_dist){
					center_candidate_dist = center_dist;
					center_candidate = centers[k];
				}
			}
			svg
			.append('rect')
			.attr('style',function(){return 'opacity:' + voronoi_opacity + ';fill:light' + center_candidate.color})
			.attr('x',i)
			.attr('y',j)
			.attr('width',voronoi_coarseness)
			.attr('height',voronoi_coarseness)
		}
	}
	document.getElementById('voronoitext').innerHTML = "<b>Complete!</b>";
}