...
NP_INT16 = lambda image: np.array([item for sublist in list(image.getdata()) for item in sublist],np.int16)
...
def mutate(polygons,gauss):
    ...
    if gauss == 'gaussian':
        ...
        if choice == 'rgb':
            ...
            value = polygon[choice]
            subindex = random.choice(range(len(value)))
            value[subindex] = random.choice(((lambda x: [RAND_INC_DEC(x[0]), x[1]]),(lambda x: [x[0], RAND_INC_DEC(x[1])])))(value[subindex])
            value[subindex] = (lambda x: [VAL_RESTRICT(width)(x[0]),VAL_RESTRICT(height)(x[1])])(value[subindex])
